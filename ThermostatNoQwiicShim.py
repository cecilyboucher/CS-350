...
#    2          Use MockAht20 if the QWIIC shim was not installed
#    3          Added code to control the red and blue LEDs for heat, cool, and off modes
#    4          Added button functionality to switch modes and change the temperature setpoint
#    5          Added display lines to show the current mode, temperature, and setpoint on the LCD
#    6          Added serial output to send thermostat status to the TemperatureServer
#------------------------------------------------------------------
...
##
import board
## Uncomment the line below if using the QWIIC Shim with an AHT20 sensor
...
##
## MockAht20 - A mock implementation of the AHTx0 temperature sensor.
## This class simulates the AHT20 sensor by returning a hardcoded temperature
## value. It is intended for use when the QWIIC Shim is not available.
##
## The 'temperature' property returns a value in degrees Celsius, matching
## the interface of the real adafruit_ahtx0.AHTx0 sensor.
##
class MockAht20:
    MOCK_TEMPERATURE_C = 20.0   ## 68 degrees Fahrenheit

    def __init__(self):
        self._temp = self.MOCK_TEMPERATURE_C
...
## Replace MockAht20() with adafruit_ahtx0.AHTx0(i2c) if using the QWIIC Shim.
##
thSensor = MockAht20()
...
class TemperatureMachine(StateMachine):
    ...
    ##
    def __init__(self):
        self.setPoint = 72      # Default setpoint 
